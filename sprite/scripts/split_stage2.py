...
# DEPRECATED 2026-04-29: Stage 2는 다리 없는 원형으로 컨셉 변경됨.
# head/leg 분리가 더 이상 필요 없음. 보존만 (재사용 가능성 0에 가까움).
...
NOBG_ROOT = Path(__file__).parent.parent / "nobg"
HEAD_ROOT = Path(__file__).parent.parent / "_archive" / "deprecated_stage2_split" / "head"
LEG_ROOT = Path(__file__).parent.parent / "_archive" / "deprecated_stage2_split" / "leg"

STAGE2_DIR = NOBG_ROOT / "stage2"

NECK_ZONE_TOP_FRAC = 0.50
NECK_ZONE_BOTTOM_FRAC = 0.85
...
def main() -> None:
    count = 0
    for png in sorted(STAGE2_DIR.rglob("*.png")):
        rel = png.relative_to(NOBG_ROOT)
        h, l, ny = process(png, HEAD_ROOT / rel, LEG_ROOT / rel)
        count += 1
        flag = "  [!]" if l < 50 or h < 500 else ""
        print(f"{str(rel):<45} neck_y={ny:>4}  head={h:>6}  leg={l:>5}{flag}")
